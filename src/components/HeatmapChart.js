...
function HeatmapChart({ reservations, courtData, value,  index  }) {
  ...
  // Increment the reservation count for each actual reservation
  reservations.forEach(reservation => {
    const reservationDate = new Date(reservation.date);
    if (reservationDate >= today) {
  ...
  const uniqueDates = [...new Set(Object.values(reservationCounts).map(item => item.date))];
  const uniqueCourtNames = [...new Set(Object.values(reservationCounts).map(item => item.courtName))];

  // Create the heatmap data in the required format for ApexCharts
  const heatmapData = uniqueCourtNames.map(courtName => ({
    name: courtName,
    data: uniqueDates.map(date => {
      const key = `${date}_${courtData.find(court => court.name === courtName).id}`;
  ...
  return (
    <div id="chart" className="chart-container">
     <p className='personalName'>Number of booked courts per day:</p>
      ...
      series={heatmapData} type="heatmap" 
       height={120}
        fill={cellColor}
        className={cellClassName} style={{ fill: cellColor }} />
