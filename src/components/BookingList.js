...
const BookingList = ({
  ...
  const handleCancelReservation = async (reservationId) => {
    ...
    try {
      ...
      const reservationToDelete = reservations.find(reservation => reservation.id === reservationId);
              ...
              <table>
                <thead>
                  <tr>
                    <th colSpan="5">
                      <p>    {userName ? (
          <div className="personalName">{formattedUserName}, нere are your booking list</div>
        ) : (
  ...
  reservations.map((reservation) => {
    ...
    const court = courtData.find(court => court.id === reservation.court_id);
